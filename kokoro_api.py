...
class EnhancedModelManager:
  ...
  def __init__(self):
      ...
      if CUDA_AVAILABLE:
          try:
              ...
              logger.warning("GPU model unavailable - falling back to CPU only")
      else:
          logger.info("GPU model not loaded (CUDA not available)")
  ...
  def generate_audio_for_chunk(self, chunk: TextChunk, voice_name: str, speed: float = 1.0, use_gpu: bool = True) -> torch.Tensor:
      ...
      effective_use_gpu = use_gpu and CUDA_AVAILABLE and (True in self.models)
      
      # Detailed device selection logging
      if use_gpu and not CUDA_AVAILABLE:
          logger.warning(f"Chunk {chunk.chunk_id}: GPU requested but CUDA not available - using CPU")
      elif use_gpu and CUDA_AVAILABLE and (True not in self.models):
          logger.warning(f"Chunk {chunk.chunk_id}: GPU requested but GPU model failed to load - using CPU")
      ...
      if effective_use_gpu:
          log_gpu_status(f"before chunk {chunk.chunk_id}")
      ...
      for _, ps, _ in pipeline(chunk.text, voice_name, speed):
          ...
          try:
              ...
              if effective_use_gpu:
                  log_gpu_status(f"after chunk {chunk.chunk_id}")
              ...
              if effective_use_gpu:
                  ...
                  try:
                      log_gpu_status("before CPU fallback")
                  ...
                  logger.error(f"CPU inference failed for chunk {chunk.chunk_id} - no fallback available")
                  raise
  ...
  def generate_audio(self, text: str, voice_name: str, speed: float = 1.0, use_gpu: bool = True) -> torch.Tensor:
      ...
      if use_gpu and not CUDA_AVAILABLE:
          ...
          logger.warning("GPU requested but GPU model failed to load - using CPU")
      ...
      # Log GPU memory before inference if using GPU
      if effective_use_gpu:
          log_gpu_status("before audio generation")
      ...
      for _, ps, _ in pipeline(text, voice_name, speed):
          ...
          try:
              ...
              # Log GPU memory after inference if using GPU
              if effective_use_gpu:
                  log_gpu_status("after audio generation")
              ...
              if effective_use_gpu:
                  ...
                  logger.error("CPU inference failed - no fallback available")
                  raise
